...
int Tcp::openClient(const char *addr, int port) {
...
int Tcp::openServer(const char *addr, int port) {
	if ((m_sock = socket(AF_INET, SOCK_STREAM, 0)) == INVALID_SOCKET) {
		logE << "socket creation failed: " << strerror(errno) << std::endl;
		return -1;
	}
...
int Tcp::listen(struct sockaddr_in &client) {
...
int Tcp::receive(char *buf, int size) {
	if (m_sock == INVALID_SOCKET) {
...
int Tcp::send(const char *buf, int size) {
	if (m_sock == INVALID_SOCKET) {
		logE << "socket unavailable" << std::endl;
		return -1;
	}
